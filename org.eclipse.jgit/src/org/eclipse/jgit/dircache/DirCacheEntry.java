...
public class DirCacheEntry {
	...
	/**
	 * Create an empty entry at stage 0 and set FileMode
	 *
	 * @param newPath
	 *            name of the cache entry.
	 * @param fileMode
	 ...
	 *             or DirCache file.
	 */
	public DirCacheEntry(String newPath, FileMode fileMode) {
		this(Constants.encode(newPath), STAGE_0);
	...
	/**
	 * Create an empty entry at the specified stage and set file mode
	 *
	 * @param newPath
	 *            name of the cache entry.
	 * @param stage
	 ...
	 *             or DirCache file.
	 */
	public DirCacheEntry(byte[] newPath, FileMode fileMode) {
		this(newPath, STAGE_0);
		setFileMode(fileMode);
	}
	 ...
	 */
	@SuppressWarnings("boxing")
	public DirCacheEntry(byte[] path, int stage, FileMode fileMode) {
		checkPath(path);
		if (stage < 0 || 3 < stage)
			throw new IllegalArgumentException(MessageFormat.format(
					JGitText.get().invalidStageForPath, stage, toString(path)));
